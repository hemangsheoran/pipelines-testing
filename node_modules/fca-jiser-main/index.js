...
function buildAPI(globalOptions, html, jar) {
    ...
    logger(`Đăng Nhập Tại ID: ${userID}`, "[ FCA-JISER ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`Vùng Của Tài Khoản Là: ${region}`, "[ FCA-JISER ]");
    } else {
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
    return function(res) {
        ...
        logger("Đang Đăng Nhập...", "[ FCA-JISER ]");
        return utils
                     ...
                     try {
                         ...
                         process.env["FBKEY"] = answer;
                             fs.writeFile('../.env', `FBKEY=${answer}`, function (err) {
                                 if (err) {
                                     logger("Tạo File ENV Thất Bại !", "[ FCA-JISER ]")
                                     rl.pause();
                                 }
                                 else logger("Tạo Thành Công File ENV !","[ FCA-JISER ]")
                                 rl.pause();
                             });
                         })
                     }
                         ...
                         console.log(e);
                         logger("Đã Có Lỗi Khi Đang Try Tạo Ra Câu Hỏi =))", "[ FCA-JISER ]");
                         rl.pause();
                     }
                 }
             },
...
/*           var axios = require('axios');
        ...
        console.log(e);
        logger("Đã Có Lỗi Khi Đang Try Tạo Random Password =))", "[ FCA-JISER ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger('Chưa Sẵn Sàng Để Decrypt Appstate !', '[ FCA-JISER ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger('Decrypt Appstate Thành Công !', '[ FCA-JISER ]');
                }
                catch (e) {
                    logger('Thay AppState Đi !', '[ FCA-JISER ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger('Thay AppState Đi !', '[ FCA-JISER ]')
        }
    }
    try { 
    ...
    return logger('Thay AppState Đi !', '[ FCA-JISER ]')
        ...
        console.log(e);
            ...
            mainPromise
                .then(function() {
                    logger('Hoàn Thành Quá Trình Đăng Nhập !', "[ FCA-JISER ]");
                        logger('Auto Check Update ...', "[ FCA-JISER ]");
            ...
            axios.get('https://pastebin.com/raw/S5Hr7QVT').then(async (res) => {
                const localbrand = JSON.parse(readFileSync('./node_modules/fca-jiser-main/package.json')).version;
                    if (localbrand != res.data.version) {
                        ...
                        log.warn("[ FCA-JISER ] •",`Tiến Hành Tự Động Cập Nhật Lên Phiên Bản Mới Nhất !`);
                            try {
                                execSync('npm install fca-jiser-main@latest', { stdio: 'inherit' });
                                logger("Nâng Cấp Phiên Bản Thành Công!","[ FCA-JISER ]")
                                logger('Đang Khởi Động Lại...', '[ FCA-JISER ]');
                            ...
                            logger(`Bạn Hiện Đang Sử Dụng Phiên Bản: ` + localbrand + ' !', "[ FCA-JISER ]"); 
