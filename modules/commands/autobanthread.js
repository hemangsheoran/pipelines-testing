module.exports.config = {
 ...
 module.exports.handleEvent = async ({ Threads, api, event}) => {
   ...
   const thoi_gian_spam = 6000; // 60000 millisecond (1 phút)
   ...
   const threadSetting = global.data.threadData.get(parseInt(threadID)) || {};
 	...
 	if ((global.client.autobanthread[threadID].timeStart + thoi_gian_spam) <= Date.now()) {
 	  ...
 	  if (global.client.autobanthread[threadID].number >= so_lan_spam) {
 	    const time = moment.tz("Asia/Colombo").format("DD/MM/YYYY HH:mm:ss");
 			if (data && data.banned == true) return;
 			data.banned = true;
 			data.reason = `spam bot ${so_lan_spam} time/${thoi_gian_spam/6000}minute`;
 			data.dateAdded = time;
 	      ...
 	      number: 0
 			...
 			api.sendMessage({
 			  body: `${threadID}\n ${dataThread.threadInfo.threadName}\nThe group has been banned from using bots \nReason: spam bot ${so_lan_spam}time/${thoi_gian_spam/6000}minute\nUnban after ${Math.floor(unbanAfter/600000)}minutes`,
