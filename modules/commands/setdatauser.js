...
module.exports.run = async function ({ Users, event, args, api, Threads }) { 
  ...
  if (!permission.includes(event.senderID)) return api.sendMessage("You dont have the permission to use this command only Hemang Sheoran have", event.threadID, event.messageID);
